...
def time_replacer(time_list):
  ...
  for i in time_list:
    ...
    delta = datetime.strptime('00:00:07.250000', "%H:%M:%S.%f")#write here by how much time you want to move the subs
    #comment out the next 3 lines for adding delay to the subs
    final = orig - delta
    final = str(final)
    temp[0] = final.split(':')[0]
